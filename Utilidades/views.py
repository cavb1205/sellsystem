...
def put_utilidad(request, pk):
    ...
    if utilidad:
        utilidad_serializer = UtilidadUpdateSerializer(utilidad, data=request.data)
        if utilidad_serializer.is_valid():
            if utilidad_valor < utilidad_serializer.validated_data['valor']:
                tienda.caja_inicial = tienda.caja_inicial - (utilidad_serializer.validated_data['valor']-utilidad_valor)
            
            elif utilidad_valor > utilidad_serializer.validated_data['valor']:
                tienda.caja_inicial = tienda.caja_inicial + (utilidad_valor - utilidad_serializer.validated_data['valor'])
...
def post_utilidad(request):
    ...
    if request.method == 'POST':
        ...
        if utilidad_serializer.is_valid():
            utilidad_serializer.save()
            tienda.caja_inicial = tienda.caja_inicial - utilidad_serializer.validated_data['valor']
            tienda.save()
            return Response(utilidad_serializer.data, status=status.HTTP_200_OK)
        return Response(utilidad_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
